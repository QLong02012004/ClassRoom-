...
export const sendPasswordSetupEmail = async (toEmail: string, otp: string) => {
    const senderEmail = process.env.SMTP_USER || 'no-reply@classroom.com';
    const mailOptions = {
        from: `"ClassRoom" <${senderEmail}>`,
        to: toEmail,
        subject: `${otp} là mã xác nhận thiết lập mật khẩu ClassRoom của bạn`,
        text: `Chào bạn,\n\nMã xác nhận OTP để thiết lập mật khẩu mới của bạn là: ${otp}\n\nMã có hiệu lực trong 10 phút. Vui lòng không chia sẻ mã này cho người khác.\n\nTrân trọng,\nĐội ngũ ClassRoom`,
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1);">
                <div style="text-align: center; margin-bottom: 20px;">
                    <h2 style="color: #f47c20; margin: 0;">ClassRoom</h2>
                ...
                <div style="background-color: #f8fafc; padding: 20px; border-radius: 8px;">
                    <h3 style="color: #0f172a; margin-top: 0;">Thiết lập Mật khẩu Lớp học</h3>
                    <p style="color: #334155; line-height: 1.5;">Chào bạn,</p>
                    <p style="color: #334155; line-height: 1.5;">Bạn đang thực hiện thiết lập mật khẩu đăng nhập trực tiếp cho tài khoản liên kết Google của mình. Mã xác nhận 6 chữ số của bạn là:</p>
    ...
    try {
        ...
        console.error("Lỗi khi gửi email thiết lập mật khẩu:", error);
        return false;
    }
};
