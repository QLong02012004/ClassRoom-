...
| **TC-ADM-05** | **Biểu đồ Tăng trưởng (User Growth)** | **05.1 (UI/UX)**: Biểu đồ cột (Bar Chart) so sánh Giáo viên & Học sinh | 1. Quan sát khu vực Biểu đồ Tăng trưởng | Dữ liệu từng tháng | Biểu đồ trực quan, phân biệt rõ cột Giáo viên & Học sinh theo màu sắc, rơ chuột vào cột hiển thị Tooltip con số chính xác. | `[ ] Pass`<br>`[ ] Fail` |
| **TC-ADM-06** | **Dòng thời gian Hoạt động gần đây (Recent Activity)** | **06.1 (Functional)**: Kiểm tra phân loại Icon & Màu sắc sự kiện | 1. Quan sát danh sách Hoạt động gần đây | Sự kiện gần nhất | Hiển thị đúng Icon & Màu sắc Semantic:<br>- Tạo lớp (Xanh dương)<br>- Bài tập (Tím)<br>- Điểm danh (Xanh lá)<br>- Tài liệu (Vàng)<br>- Thông báo (Đỏ) | `[ ] Pass`<br>`[ ] Fail` |
...
| | | **06.3 (Navigation)**: Thao tác nút "Xem tất cả" | 1. Click nút "Xem tất cả" ở chân thẻ Hoạt động gần đây | Click xem thêm | Mở ra danh sách đầy đủ hoặc cuộn xem tất cả các hoạt động trên hệ thống. | `[ ] Pass`<br>`[ ] Fail` |
...
| | | **06C.3 (Interactive)**: Chuyển trạng thái Đã đọc & Đóng Popover | 1. Click vào 1 thông báo chưa đọc trong Popover<br>2. Click ra ngoài vùng Popover | Click thông báo & Click outside | - Thông báo chuyển từ nền cam nhạt sang trắng, dấu chấm đỏ biến mất.<br>- Click ra ngoài bất kỳ vị trí nào sẽ tự động đóng Popover. | `[ ] Pass`<br>`[ ] Fail` |
...
| Mã TC | Tên Kịch Bản | Trường Hợp Kiểm Thử (Sub-Cases) | Các Bước Thực Hiện | Dữ Liệu Đầu Vào | Kết Quả Mong Đợi (Expected Outcome) | Trạng Thái |
| :---: | :--- | :--- | :--- | :--- | :--- | :---: |
| **TC-ADM-07** | **Tạo Giáo viên mới (Positive)** | **07.1 (Positive)**: Tạo tài khoản Giáo viên hợp lệ | 1. Click "Thêm giáo viên mới"<br>2. Nhập Tên, Email, Mật khẩu chuẩn<br>3. Bấm "Tạo tài khoản" | Họ tên: `Lê Văn B`<br>Email: `teacherb@gmail.com`<br>Pass: `123456` | - Toast: "Tạo tài khoản thành công".<br>- Bảng tự động xuất hiện Giáo viên mới với Badge xanh `Giáo viên`. | `[ ] Pass`<br>`[ ] Fail` |
...
| | | **08.3 (Boundary)**: Mật khẩu quá ngắn (< 6 ký tự) | 1. Nhập Mật khẩu 3 ký tự | Pass: `123` | - Hiển thị lỗi validation: "Mật khẩu phải có ít nhất 6 ký tự!". | `[ ] Pass`<br>`[ ] Fail` |
...
| **TC-ADM-11** | **Lọc theo Trạng thái** | **11.1 (Filter)**: Lọc tài khoản Active / Locked | 1. Chọn Lọc `Active` hoặc `Locked` | Select Status | Bảng lọc chính xác các tài khoản đang hoạt động hoặc đang bị khóa. | `[ ] Pass`<br>`[ ] Fail` |
